...
class UNICACHE_MemcachedCache extends UNICACHE_Cache
{
    ...
    {
        if (class_exists('Memcache', false) && ($this->connection instanceof Memcache))
        {
            $this->connection->close();
        }
        elseif (class_exists('Memcached', false) && ($this->connection instanceof Memcached) && method_exists($this->connection, 'quit'))
        {
    ...
    public function put($key, $data, $ttl)
    {
        ...
        {
            return $this->connection->set($key, $value, (int)$ttl);
        }
        elseif (get_class($this->connection) === 'Memcache')
        {
            return $this->connection->set($key, $value, 0, (int)$ttl);
        }
        return false;
    }
